...
from .ephemeris import (
    PLANET_IDS, SIGNS, sign_of, whole_sign_house_of, local_to_julian_ut,
)
...
def _transiting_house(jd_ut, planet_name, asc_sign):
    if planet_name == "Ketu":
        (lon, *_r), _f = swe.calc_ut(jd_ut, swe.MEAN_NODE, swe.FLG_SIDEREAL)
        lon = (lon + 180) % 360
    else:
        (lon, *_r), _f = swe.calc_ut(jd_ut, PLANET_IDS[planet_name], swe.FLG_SIDEREAL)
...
def house_change_calendar(start: datetime, months: int, latitude=None, longitude=None,
    ...
    while cursor <= end:
        ...
        for body in bodies:
            if body == "Ketu":
                (lon, lat_, dist_, lon_speed, *_rest), _f = swe.calc_ut(jd, swe.MEAN_NODE, swe.FLG_SIDEREAL | swe.FLG_SPEED)
                lon = (lon + 180) % 360
                retro = lon_speed >= 0  # Ketu retrograde flag mirrors Rahu's (inverted)
            else:
                (lon, lat_, dist_, lon_speed, *_rest), _f = swe.calc_ut(jd, PLANET_IDS[body], swe.FLG_SIDEREAL | swe.FLG_SPEED)
                retro = lon_speed < 0
            sign, _deg = sign_of(lon)
