...
def build_narrative_pdf(output_path, lang, birth_params, chart, narrative_report, section_order,
                         provider_label):
    """Renders a Ganesha-style narrative report: intro, sections broken into
    quarters with prose paragraphs, practical notes, conclusion."""
    t = get_translations(lang)
    st = _styles()
    doc = SimpleDocTemplate(
        output_path, pagesize=A4,
    ...
    story.append(Paragraph(t["narrative_title"], st["title"]))
    story.append(Paragraph(
        f'{t["ascendant"]}: {t["signs"].get(chart["ascendant"]["sign"], chart["ascendant"]["sign"])} '
        f'{chart["ascendant"]["sign_deg"]:.2f}° &nbsp;|&nbsp; {t["narrative_provider_label"]}: {provider_label}',
        st["subtitle"],
    ))

    if narrative_report.get("intro"):
        story.append(Paragraph(t["narrative_intro_label"], st["h2"]))
        story.append(Paragraph(narrative_report["intro"], st["body"]))
    ...
    for key in section_order:
        ...
        if not entries:
            continue
        story.append(Paragraph(t.get(f"section_{key}", key.title()), st["h2"]))
        for entry in entries:
            quarter_style = ParagraphStyle(
                "quarter", parent=st["body"], fontName="DejaVuSans-Bold", fontSize=9,
                spaceBefore=6, spaceAfter=2,
            )
            story.append(Paragraph(entry.get("quarter", ""), quarter_style))
            story.append(Paragraph(entry.get("text", ""), st["body"]))

    if narrative_report.get("practical_notes"):
        story.append(Paragraph(t["narrative_practical_notes"], st["h2"]))
        story.append(Paragraph(narrative_report["practical_notes"], st["body"]))

    if narrative_report.get("conclusion"):
        story.append(Paragraph(t["narrative_conclusion"], st["h2"]))
        story.append(Paragraph(narrative_report["conclusion"], st["body"]))
